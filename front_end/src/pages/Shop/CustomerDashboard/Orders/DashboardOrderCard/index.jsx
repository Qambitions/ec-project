...
export default function DashboardOrderCard({ orderInfo }) {
  ...
  const fetchOrderDetail = async () => {
    ...
    if (res.data.exitcode === 0) {
      {orderInfo.trang_thai.localeCompare("ĐÃ XÁC NHẬN") ? (
        console.log("Đã xác nhận")
      ) : orderInfo.trang_thai.localeCompare("CHỜ XÁC NHẬN") ? (
        console.log("Chờ xác nhận")
      ) : orderInfo.trang_thai.localeCompare("ĐANG GIAO") ? (
        console.log("Đang giao")
      ) : orderInfo.trang_thai.localeCompare("ĐÃ GIAO") ? (
        console.log("Đã giao")
      ) : (
        console.log("Đã hủy")
      )}
    ...
    <div className="dashboard__ordercard">
      <div className="container__spacebetween">
        <div className="container__flex">
          ...
          ) : (orderInfo.trang_thai.localeCompare("ĐÃ HỦY")===0) || (orderInfo.trang_thai.localeCompare("THANH TOÁN THẤT BẠI")===0) ?(
            <label className="canceled_label">Đã hủy</label>
          ) :<label className="canceled_label">Đã hủy</label>}
          <label>|</label>
          <label>Tổng tiền: {orderInfo.tong_phi}đ</label>
        </div>
        <label className="navigate__detail" onClick={handleViewDetail}>Xem chi tiết  {'>>'}</label>
      </div>
      <hr/>
      <div>
        {items.map((item) => (        
          <OrderCard so_luong_mua={item.so_luong_mua} ten_npp={item.ten_npp} hinh_anh={item.hinh_anh} tensp={item.ten_sp} gia_phai_tra={item.gia_phai_tra}/>
        ))}
      </div>
    </div>
  );
